...
| YOLO 학습/추론 | `ultralytics` — `YOLO("yolo26m.pt")` |
...
- 추출은 기본 누적 방식이다. 기존 프레임·라벨을 보존하며 `frame_<source_id>_<index>.jpg`에 저장한다. 소스 ID는 기존 파일과 `sources.json`을 확인해 충돌 없이 발급한다. `sources.json`은 원자적으로 갱신하고 정상 완료·취소·오류 시 실제 저장 장수와 상태를 기록한다.
...
- 기본 `미처리만`은 라벨 파일이 없는 프레임만 처리한다. `선택 소스 재라벨링`은 선택 프레임의 추론 성공 후 해당 라벨을 원자적으로 교체한다. 사전 일괄 삭제는 없다. 빈 라벨은 처리 완료 배경 이미지다.
...
- **클래스 편집기 (리스트형)**: 탭 진입/「클래스 불러오기」 시 `scan_classes()`로 라벨을 스캔해 **클래스별 행(ID · 프레임 수 + 즉시 수정 가능한 이름 입력칸)** 을 `@gr.render`로 동적 생성 → 가독성·편의성 향상. 이름 칸은 `gr.Textbox(interactive=True)` (render 동적 생성 시 Gradio 6.x가 입력 추론을 못 해 비활성화되는 것 방지)
  - `pipeline/data_store.py`가 `dataset/classes.json`의 영구 ID·이름·프롬프트 별칭을 관리한다. SAM3의 프롬프트별 로컬 ID를 영구 ID로 변환해 저장한다. 새로운 이름만 ID 뒤에 추가한다.
  - 기존 YAML의 names로 이전 클래스 정보를 이관한다. 알 수 없는 ID는 `class_<id>`로 표시하고 이름을 지정하기 전 누적 라벨링을 막는다. 최신 프롬프트로 과거 라벨의 의미를 추측하지 않는다.
  - 이름 칸의 `.input`에서 영구 목록을 저장하고 `ds_prompts`를 갱신한다. 편집 중 render state는 바꾸지 않는다. 탭 진입 시 영구 목록을 다시 읽어 신규 클래스와 저장된 편집을 반영한다.
  - ID가 관측되지 않아도 영구 목록의 모든 행을 표시해 sparse ID의 순서 압축을 방지한다. 프레임 수는 최상위 라벨에서 클래스별 프레임 단위로 집계한다.
- `load_preview(prompts_str, filter_empty)`: raw_frames + labels 매칭 → bbox 오버레이 Gallery 표시
- 통계: 전체 / 라벨 있음 / 라벨 없음 프레임 수
- "빈 라벨 프레임 숨기기 / 학습에서 제외" 체크박스는 기본 활성화한다. 원본 이미지와 SAM3 빈 라벨은 보존하며 미리보기·데이터셋 구성에서 제외한다. 체크를 끄면 처리 완료된 빈 라벨 이미지를 배경으로 포함한다.
...
- 미처리 이미지는 학습에서 제외한다. 소스 단위로 분리하며 validation을 직접 지정하지 않으면 `split_manifest.json`의 기존 배정을 유지한다. 같은 YouTube 영상 및 내용 해시가 같은 새 비디오 수집분은 같은 분할에 둔다.
- 임시 폴더에서 이미지·라벨 복사 및 라벨 검증을 완료한 후 train/val·YAML·분할 기록을 교체한다. 교체 중 예외 발생 시 이전 폴더·YAML·라벨 캐시를 복원한다. 강제 프로세스 종료에 대한 다중 파일 원자성은 보장하지 않는다.
- `dataset/dataset.yaml`은 안전한 YAML 직렬화로 생성하며 영구 클래스 목록과 다른 오래된 UI 입력은 거부한다.
- Gradio의 추출·라벨 미리보기/실행·클래스 편집·삭제·구성 이벤트는 `dataset_write` concurrency 그룹을 공유한다.
...
- 모델: `yolo26m.pt` (기본 베이스) — **베이스 모델 드롭다운으로 이어학습(파인튜닝) 지원**: 비우면(값 `""`) 사전학습 `yolo26m.pt`로 처음부터, 학습된 모델(`best.pt`)을 고르면 그 가중치 위에 `YOLO(base_path)`로 로딩 후 새 학습. 드롭다운 첫 항목이 "처음부터"이고 이후는 `models.list_trained_models()` 재사용(추론 탭과 동일). 결과는 항상 `name` 입력칸이 정한 새 폴더에 저장돼 베이스는 보존
...
| batch | -1 | CUDA GPU 메모리 기준 자동 설정. CPU·MPS에서는 기본 16 사용. 드롭다운에서 -1 또는 1~64 선택 |
...
- 이미지 폴더 모드(`_predict_folder`): 업로드된 이미지를 순회하며 장당 추론·표시, 중지 전까지 반복(기본 1초 주기, 추론 소요 시간을 뺀 나머지 시간 대기)
...
- `detection_interval`: 실제 경과 시간 기준 탐지 주기(초), 기본값 1.0. 그 사이에는 마지막 bbox를 재사용한다.
...
- 접속 기기 웹캠은 추론 탭과 동일하게 원본 영상 위에 SVG 박스·영역을 겹친다. 별도의 결과 이미지는 숨기며, `스트림 시작`에서 카메라 전송을 자동 시작해 Gradio 녹음 버튼은 표시하지 않는다. 전체화면에서도 영상과 오버레이를 함께 확대한다.
- `자동 영역 추적 클래스`는 선택한 모델의 `names`에서 실제 클래스명·ID를 불러온다. 다중 클래스 모델은 사용자가 경계 객체를 선택하며, `Safety Cone` 이름 필터 없이 선택한 클래스 ID의 Track 3개 이상으로 자동 영역을 생성·갱신한다. 자동 영역 이름은 `<학습 클래스명> zone`이다.
...
- `detection_interval`: 실제 경과 시간 기준 탐지·ByteTrack 갱신 주기(초), 기본값 1.0. 추적 영역이 활성화되어도 같은 주기를 유지한다.
- `pipeline/media.py`의 `video_frames()`가 파일·YouTube 입력을 소스 FPS에 맞춰 읽는다. 탐지 사이에도 영상은 갱신하고, 고정 편집용 프레임·Track은 마지막 탐지 시점의 쌍을 유지한다.
...
- 라바콘 추적 모드는 `YOLO.track(..., persist=True, tracker="bytetrack.yaml")`를
  ...
  중앙을 anchor로 저장하며, 추적 영역이 활성화되어도 설정한 초 단위 탐지 주기를 유지한다.
