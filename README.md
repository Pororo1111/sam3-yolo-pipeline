...
- 웹캠은 기존 화면 흐름(웹캠 장비 선택/새로고침)을 유지하되, 실제 프레임은 브라우저 권한으로 전달되는 `WebRTC` 컴포넌트에서 받습니다. 서버 컴퓨터의 카메라 장비 목록(`/dev/video*`, OpenCV 인덱스)은 탐색하지 않습니다.
...
- 웹캠 추론도 브라우저 권한으로 전달되는 프레임을 사용하며, 서버 카메라 장비 목록은 탐색하지 않습니다.
...
- 웹캠 침입 감지도 브라우저 권한으로 전달되는 프레임을 사용하며, 서버 카메라 장비 목록은 탐색하지 않습니다.
...
웹캠 소스는 FastRTC/Gradio `WebRTC` 컴포넌트로 입력과 출력을 처리합니다. 이전 `gr.Image(..., streaming=True)` 이벤트 기반 fake streaming 경로는 웹캠 화면부에서 제거했고, 서버 컴퓨터의 카메라 장비 목록(`/dev/video*`, OpenCV camera index)은 계속 탐색하지 않습니다.

### 적용 범위

- **WebRTC 적용 대상은 `웹캠` 소스만**입니다.
...
- 적용 탭은 실시간 소스를 다루는 **Tab 1 프레임 추출**, **Tab 5 추론**, **Tab 6 침입 감지**입니다. Tab 2~4는 저장된 데이터/학습 흐름이라 WebRTC 대상이 아닙니다.

### UI/UX 원칙

- 기존 `소스` 라디오의 4가지 옵션은 유지합니다.
- 기존 웹캠 화면 흐름(`웹캠 장비` 선택 영역, `웹캠 목록 새로고침`, 시작/중지 버튼, 상태창)은 유지합니다.
- 내부 웹캠 프레임 수신/표시 컴포넌트만 FastRTC/Gradio WebRTC 기반으로 교체했습니다.
- 서버 컴퓨터의 카메라 장비 목록(`/dev/video*`, OpenCV camera index)은 계속 탐색하지 않습니다.
...
| 웹캠 | **WebRTC + FPS 저장** | **WebRTC + YOLO** | **WebRTC + YOLO + zone overlay** |
